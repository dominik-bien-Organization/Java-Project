...
public class PatientPageController implements Initializable {
    ...
    private String patientId = "patient123"; // To ID powinno być pobrane po zalogowaniu

    @Override
    public void initialize(URL url, ResourceBundle resourceBundle) {
        try {
            // Połącz się z serwerem i zacznij nasłuchiwać
            client = new ClinicClient("localhost", 12345, this::handleServerMessage);
            client.startListening();

            // Zaloguj się na serwerze
            client.sendMessage("LOGIN:PACJENT:" + patientId);
    ...
    private void handleBookAppointment() {
        ...
        if (doctorId.isEmpty() || date.isEmpty() || time == null) {
            // Pokaż błąd
            return;
        }

        // Format wiadomości: BOOK_APPOINTMENT:<ID_LEKARZA>:<ID_PACJENTA>:<DATA>:<GODZINA>
        String message = "BOOK_APPOINTMENT:" + doctorId + ":" + this.patientId + ":" + date + ":" + time;
        client.sendMessage(message);
    }

    // Metoda, która obsługuje wiadomości od serwera
    private void handleServerMessage(String message) {
        // Ważne: operacje na GUI muszą być wykonane w wątku JavaFX
        Platform.runLater(() -> {
            String[] parts = message.split(":", 2);
            String command = parts[0];
            String data = parts[length > 1 ? parts[1] : "";

            if (command.equals("BOOKING_CONFIRMED")) {
                ...
                statusLabel.setText("Błąd: " + data);
            }
        });
    }

    // Pamiętaj o zamknięciu połączenia, gdy okno jest zamykane!
