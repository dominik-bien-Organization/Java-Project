...
public class DoctorPageController implements Initializable {
    ...
    public void loginAccount() {
        ...
        try {
            Optional<Doctor> doctorOpt = doctorService.login(email, password);
            if (doctorOpt.isPresent()) {
                alert.successMessage("Logowanie wykonano pomyślnie!");
                openDoctorDashboard(doctorOpt.get(), clinicClient);
            } else {
    ...
    private void openDoctorDashboard(Doctor doctor, ClinicClient clinicClient) {
        try {
            FXMLLoader loader = new FXMLLoader(getClass().getResource("/com/example/clinicapp/client/DoctorDashboard.fxml"));
